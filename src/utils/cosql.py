...
def cosql_get_input(
    ...
    serialize_utterances = sep.join(utterances)
    return  prefix + serialize_utterances + " | " + serialized_schema.strip()
...
def cosql_pre_process_function(
    ...
    if data_training_args.use_decomposition:
        ...
        if len(batch['utterances']) == 1000:
            count = 0
        else:
            count = 1000
        for question, serialized_schema, db_id, query, db_column_names in zip(batch["utterances"], batch["serialized_schema"], batch["db_id"], batch["query"], batch["db_column_names"]):
            input_str = cosql_get_input(utterances=question, serialized_schema=serialized_schema, prefix=prefix)
            ...
            for idx, tok in enumerate(query_tok_list):
                ...
                if '(' in query_tok_list[idx] and ')' in query_tok_list[idx] and ('sum' in query_tok_list[idx] or 'count' in query_tok_list[idx] or 'avg' in query_tok_list[idx] or 'max' in query_tok_list[idx] or 'min' in query_tok_list[idx]):
                    res = pattern_1.split(query_tok_list[idx])
                    query_tok_list[idx:idx+1] = res
                elif '(' in query_tok_list[idx] and 'select' in query_tok_list[idx]:
    ...
    def _post_process_function(
        ...
        if self.stage == 'content':
            ...
            with open(f"{self.args.output_dir}/content_{stage}.json", "w") as f:
                json.dump(
                    [dict(**{"input": meta['context']}, **{"prediction": prediction}, **{"label": label}, **{"score": prediction==label}, **{"pred_sql": final_pred_sql}, **{"gold_sql": meta['query'], **{"turn_idx": meta["turn_idx"]}}) for meta, prediction, final_pred_sql, label in zip(metas, predictions, final_pred_sqls, decoded_label_ids)],
                    f,
                    indent=4,
                )
