...
from .geoquery import geoquery_add_serialized_schema, geoquery_pre_process_function
...
def load_dataset(
    ...
    _spider_add_serialized_schema = lambda ex, mode: spider_add_serialized_schema(
        ex=ex,
        mode=mode,
        data_training_args=data_training_args,
    )
    _spider_pre_process_function = lambda batch, max_source_length, max_target_length, mode: spider_pre_process_function(
        batch=batch,
    ...
    _geoquery_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths["geoquery"], cache_dir=model_args.cache_dir
    )
    _geoquery_metric: Callable[[], Metric] = lambda: datasets.load.load_metric(
        path=data_args.metric_paths["spider"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )
    _geoquery_add_serialized_schema = lambda ex, mode: geoquery_add_serialized_schema(
        ex=ex,
        mode=mode,
        data_training_args=data_training_args,
    )
    _geoquery_pre_process_function = lambda batch, max_source_length, max_target_length, mode: geoquery_pre_process_function(
        batch=batch,
        max_source_length=max_source_length,
        max_target_length=max_target_length,
        mode=mode,
    ...
    _cosql_add_serialized_schema = lambda ex, mode: cosql_add_serialized_schema(
        ex=ex,
        mode=mode,
        data_training_args=data_training_args,
    )
    _cosql_pre_process_function = lambda batch, max_source_length, max_target_length, mode: cosql_pre_process_function(
        batch=batch,
        max_source_length=max_source_length,
        max_target_length=max_target_length,
        data_training_args=data_training_args,
    ...
    elif data_args.dataset == "geoquery":
        metric = _geoquery_metric()
        dataset_splits = prepare_splits(
            dataset_dict=_geoquery_dataset_dict(),
            add_serialized_schema=_geoquery_add_serialized_schema,
            pre_process_function=_geoquery_pre_process_function,
            **_prepare_splits_kwargs
        )       
